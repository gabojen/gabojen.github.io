...
aiErrCard=function(e,title,retryFn){const cancelled=e?.code==='travel/cancelled',quota=e?.code==='travel/quota';if(quota)return '<div class="card"><h4>AI 사용 횟수를 모두 썼어요</h4><p class="muted small">'+esc(e.message)+'</p></div>';return '<div class="card"><h4>'+esc(cancelled?'요청을 취소했어요':title||'추천을 불러오지 못했어요')+'</h4><p class="muted small">'+esc(cancelled?'다시 필요할 때 시작해 주세요.':'잠시 후 다시 시도해 주세요. 일정을 직접 추가할 수도 있어요.')+'</p><button class="btn ghost sm" onclick="'+esc(retryFn)+'">다시 시도</button></div>';};
aiFailReason=function(message,title){const text=String(message||'');if(/AI \d+회를 모두 썼어요|여행을 연 뒤 AI/.test(text))return {t:'AI 사용 횟수를 모두 썼어요',d:text};if(/취소/.test(text))return {t:'요청을 취소했어요',d:'필요할 때 다시 시작해 주세요.'};if(/quota|429|rate|exhaust/i.test(text))return {t:'AI 요청이 잠시 많아요',d:'잠시 후 다시 시도하거나 일정을 직접 입력해 주세요.'};if(/network|fetch|offline/i.test(text))return {t:'인터넷 연결을 확인해 주세요',d:'연결한 뒤 다시 시도할 수 있어요.'};return {t:title||'AI가 내용을 정리하지 못했어요',d:'잠시 후 다시 시도해 주세요. 계속되면 문의 · 신고로 알려 주세요.'};};
...
openTerms=function(){policyBody('이용약관',`
  ...
  <h4>5. AI·여행 정보</h4><p>AI 기능(일정 짜기·빈 시간 추천·예약 문자와 사진 읽기·학교 서류 초안)은 여행 1건당 구성원 1명이 ${Number(GABOJEN_RELEASE.aiPerTrip)||5}회까지 사용할 수 있으며, 서비스 사정에 따라 조정될 수 있습니다. AI 일정과 장소 추천은 검토가 필요한 초안이며 예약 완료나 운영 여부를 보장하지 않습니다. 교통·숙박·입장권의 실제 이용 조건, 영업시간·휴무·요금은 제공처에서 확인해 주세요. 학교 서류의 제출 기한과 양식은 각 학교 기준을 따릅니다.</p>
...
openPrivacy=function(){policyBody('개인정보 처리방침',`
  ...
  <h4>3. 처리위탁·국외 이전</h4><p>인증 및 여행 데이터 보관에 Google Firebase, AI 요청에 Google Gemini를 사용합니다. Firebase Authentication의 인증 정보(이메일·암호화된 비밀번호)는 미국에 있는 Google 서버에서 처리되며, 여행·사진 데이터베이스는 서울에 있습니다. AI 요청 내용은 Google Gemini 서버로 전송되어 처리됩니다. 이전 방법은 암호화된 인터넷 전송이며, 보유 기간은 서비스 이용 기간(탈퇴 시 삭제)입니다. Google은 인증 정보 삭제 요청 후 운영·백업 시스템에서 제거하는 데 최대 180일이 걸릴 수 있다고 안내합니다. 보관 지역: ${esc(GABOJEN_RELEASE.privacyRegion||'운영 설정 확인 중')}.</p>
  <h4>4. AI 사용</h4><p>AI 기능을 사용할 때 해당 기능에 필요한 여행지·일정 또는 선택한 예약 문자·사진·작성 문장을 Google Gemini에 전송합니다. 이 서비스는 Google의 무료 AI 요금제를 사용하며, 제공사 조건상 무료 AI에 보낸 입력과 출력은 Google의 제품 개선과 검토에 사용될 수 있습니다. 따라서 AI 기능에는 이름·연락처·예약번호·학생 실명 같은 개인정보를 넣지 말고, 사진·원문의 불필요한 부분은 먼저 가려 주세요. AI 기능은 사용자가 직접 누를 때만 실행되며, 사용 전 매번 이 내용을 안내합니다.</p>
