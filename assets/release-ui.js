...
openIntro=function(){if(!ME.uid||location.hash.startsWith('#invite='))return;previousOpenIntro();const intro=$('intro');if(intro){intro.setAttribute('role','dialog');intro.setAttribute('aria-modal','true');intro.setAttribute('aria-label','여행가보젠 처음 사용 안내');intro.querySelector('button')?.focus();}};
