...
window.onAuthed=function(user,profile){originalAuthed(user,profile);if(location.hash.startsWith('#invite='))setTimeout(joinPrompt,700);};
const originalSignedOut=window.onSignedOut;
window.onSignedOut=function(){closeOv();closeIntro();activeInvite=null;window.__editTripDraft=null;window.__editorialPlace=null;releaseState.aiConsent=false;releaseState.failed.clear();releaseState.pending=0;IS_ADMIN=false;watchingTrip=null;window.__plans=null;window.__gapList=null;window.__planTripId=null;applyDraft={school:'',grade:'',cls:'',no:'',name:''};reportDraft={name:'',did:'',learned:'',felt:'',fact:'',captions:[]};originalSignedOut();document.querySelectorAll('.screen:not(#login)').forEach(x=>x.innerHTML='');};

// Serialize repeat taps on mutation buttons, including taps before server acknowledgement.
for(const name of ['saveCreateTrip','saveAddItem','saveEditItem','savePropose','addComment','doDelItem','delProp','savePlan','commitPlan','doJoin','doDeleteTrip','delItemPhoto','delMemPhoto']){
...
openTerms=function(){policyBody('이용약관',`
  ...
  <h4>4. 이용 제한과 이의제기</h4><p>위반 내용과 정도에 따라 게시물 삭제 또는 이용 제한이 적용될 수 있습니다. 긴급한 보안 위협·명백한 불법 정보는 우선 제한 후 안내할 수 있습니다. 조치 사유와 이의제기 방법을 알리고, 접수된 소명 자료를 확인한 뒤 결과를 안내합니다. 구체적인 처리 기간과 연락처는 공개 출시 전에 공지합니다.</p>
...
openPrivacy=function(){policyBody('개인정보 처리방침',`
  ...
  <h4>3. 처리위탁·국외 이전</h4><p>인증 및 여행 데이터 보관에 Google Firebase, AI 요청에 Google Gemini를 사용합니다. 실제 보관 지역·수탁자 계약·이전 항목·시기·방법·보유 기간과 적용 근거는 공개 출시 전에 확인하여 고지해야 합니다. 현재 확인된 보관 지역: ${esc(GABOJEN_RELEASE.privacyRegion||'운영 설정 확인 중')}.</p>
  <h4>4. AI 사용</h4><p>AI 기능을 사용할 때 해당 기능에 필요한 여행지·일정 또는 선택한 예약 문자·사진·작성 문장을 Google Gemini에 전송합니다. 사진·원문에는 개인정보가 포함될 수 있으므로 불필요한 부분을 먼저 가려 주세요. 학습 이용 여부와 보유 조건은 운영자가 선택한 제공자 계약과 서비스 설정을 확인해 확정합니다.</p>
  ...
  <h4>8. 안전조치와 담당자</h4><p>인증, 여행 구성원 접근 규칙, 전송 구간 암호화, 기기 저장 최소화를 적용합니다. App Check 적용 범위와 실제 서비스 설정은 출시 점검에서 확인합니다.</p><p>운영자: ${esc(GABOJEN_RELEASE.operatorName||'공개 출시 전 확정 예정')}<br>개인정보 담당자: ${esc(GABOJEN_RELEASE.privacyContact||'공개 출시 전 확정 예정')}<br>문의: ${esc(GABOJEN_RELEASE.supportEmail||'공개 출시 전 확정 예정')}</p>
  <button class="btn ghost sm" onclick="clearLocalData()">기기에 저장된 입력 정보 지우기</button>`);};
...
renderGo=function(){
  ...
  $('go').innerHTML=`<header class="page-lead"><span class="section-label">FIND YOUR NEXT JOURNEY</span><h2>${esc(NEWS.title)}</h2><p>${esc(NEWS.lead)}</p></header><div class="rchips">${regions.map(r=>`<button class="rchip${selected===r?' on':''}" onclick="goPick('${r}')">${r}</button>`).join('')}</div><div class="discovery-grid">${places.map(p=>`<button class="discovery-card" onclick="openEditorial('${p.id}')"><div class="discovery-image" style="background-image:url('${p.image}')"><span>${esc(p.region)}</span></div><div class="discovery-copy"><small>${esc(p.tag)}</small><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p><span class="discovery-more">여행 아이디어 보기 ${svg('i-right')}</span></div></button>`).join('')}</div><p class="muted small" style="margin-top:20px">공식 관광 안내를 참고한 기본 여행 아이디어예요. 운영시간·입장 조건은 방문 전 공식 안내를 확인해 주세요. 카드 사진은 여행 분위기를 보여주는 이미지로 실제 장소와 다를 수 있어요.</p>`;
};
...
const previousOpenIntro=openIntro;
openIntro=function(explicit=false){if(!explicit&&(!ME.uid||location.hash.startsWith('#invite=')))return;previousOpenIntro();const intro=$('intro');if(intro){intro.setAttribute('role','dialog');intro.setAttribute('aria-modal','true');intro.setAttribute('aria-label','여행가보젠 처음 사용 안내');intro.querySelector('button')?.focus();}};
